...
- Subagents should prefer MCP tools automatically:
  - Planning/step sequencing (orchestrator): may use `Sequential Thinking MCP`
  - Docs/research (context-fetcher): leverage `Exa MCP` → `Ref MCP`
  - Git branching/PRs (git-workflow): prefer `GitHub MCP`
  - Dev diagnostics (external-delegator): `DigitalOcean MCP`
  - UI smoke checks (test-runner): `Playwright MCP`
  - Error insights (validator/sentry-integrator): `Sentry MCP`
...
<step number="1" subagent="project-manager" name="task_assignment">

### Step 1: Task Assignment

Use the project-manager subagent to identify which tasks to execute from the spec (using spec_srd_reference file path and optional specific_tasks array), defaulting to the next uncompleted parent task if not specified.
...
<instructions>
  ACTION: Use project-manager subagent to identify task(s) to execute
...
Use the context-fetcher subagent to gather minimal context for task understanding by always loading spec tasks.md, and conditionally loading @.agent-os/product/mission-lite.md, spec-lite.md, and sub-specs/technical-spec.md if not already in context. For any external references required, the context-fetcher should coordinate with external-delegator to use `Exa MCP` for discovery and `Ref MCP` for fetching the exact docs.
...
<parallel_subagents>
  <!-- Run support agents in parallel while the loop progresses -->
  <agent name="validator" purpose="continuous quality">
    - Perform ongoing code review (style, security, performance)
    - Watch lint/build/unit tests incrementally where possible
    - Surface failures early; feed findings back to implementer
    - If a PR exists, coordinate with git-workflow/external-delegator to post a summarized review via GitHub MCP
  </agent>
  <agent name="project-manager" purpose="task hygiene">
    - Keep tasks.md synchronized as subtasks complete
    - Flag blockers and annotate with numbered notes
  </agent>
  <agent name="orchestrator" purpose="pipeline flow">
    - Prepares next-parent-task context while current task is running
    - Coordinates PR/branch readiness via git-workflow subagent
  </agent>
  <agent name="external-delegator" purpose="MCP-first tooling">
    - Routes GitHub/DO/Exa/Ref/Playwright/Sentry/Neo4j work to MCP tools
  </agent>
</parallel_subagents>
