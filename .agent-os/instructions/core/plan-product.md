...
- Subagents should prefer MCP tools automatically:
  - Planning (orchestrator): may leverage `Sequential Thinking MCP` for stepwise plans/revisions
  - Research/docs (context-fetcher): discover via `Exa MCP`, fetch via `Ref MCP`
  - Repo/PRs/issues (git-workflow): prefer `GitHub MCP` for development workflows
  - Dev diagnostics (external-delegator): use `DigitalOcean MCP` for dev/staging checks (no prod deploys)
  - UI validation (test-runner): coordinate `Playwright MCP` for flows/assertions
  - Error analysis (validator/sentry-integrator): query `Sentry MCP` for issues/perf/Seer
  - Graph work (external-delegator): use `Neo4j Cypher MCP` for schema/queries
...
<parallel_planning>
  <agent name="orchestrator" purpose="pipeline setup">
    - Stage GitHub MCP issue shells for roadmap items (optional)
    - Prepare Exa searches for each feature area; queue Ref pulls
  </agent>
  <agent name="project-manager" purpose="tasks bootstrap">
    - Initialize tasks templates referencing the roadmap items
  </agent>
  <agent name="external-delegator" purpose="MCP routing">
    - Validate DigitalOcean dev environments exist for early diagnostics (no deploy)
  </agent>
</parallel_planning>
</process_flow>
