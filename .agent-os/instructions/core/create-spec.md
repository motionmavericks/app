...
- Subagents should prefer MCP tools automatically:
  - Planning (orchestrator): may use `Sequential Thinking MCP` to outline and iterate
  - Research/docs (context-fetcher): discover via `Exa MCP`, fetch via `Ref MCP`
  - Repo linkage (git-workflow): reference issues/PRs via `GitHub MCP` when applicable
  - Dev assumptions (external-delegator): validate via `DigitalOcean MCP` (dev only)
  - UI testability (test-runner): consider `Playwright MCP` for test planning
  - Observability (validator/sentry-integrator): incorporate `Sentry MCP` insights
...
Use the context-fetcher subagent to read @.agent-os/product/mission-lite.md and @.agent-os/product/tech-stack.md only if not already in context to ensure minimal context for spec alignment. For external context, the context-fetcher should coordinate with external-delegator to:
  - Discover references via `Exa MCP`
  - Fetch specific docs via `Ref MCP`
...
<parallel_spec_workflows>
  <agent name="validator" purpose="spec linting">
    - Check markdown structure and required sections present
  </agent>
  <agent name="orchestrator" purpose="linkage">
    - Link spec to existing GitHub MCP issues/PRs where relevant
  </agent>
  <agent name="external-delegator" purpose="evidence">
    - Attach Exa/Ref citations for critical technical choices
  </agent>
</parallel_spec_workflows>
