...
- Subagents should prefer MCP tools automatically:
  - Repo introspection (git-workflow/context-fetcher): prefer `GitHub MCP`
  - Docs lookup (context-fetcher): discover via `Exa MCP`, fetch via `Ref MCP`
  - Env/services (external-delegator): use `DigitalOcean MCP` (dev/staging)
  - UI smoke (test-runner): coordinate `Playwright MCP` for local flows
  - Error/perf (validator/sentry-integrator): query `Sentry MCP`
  - Planning (orchestrator): may employ `Sequential Thinking MCP`
...
<step number="1" subagent="orchestrator" name="analyze_existing_codebase">
...
<step number="3" subagent="orchestrator" name="execute_plan_product">
...
<step number="4" subagent="project-manager" name="customize_generated_files">
...
<step number="5" subagent="validator" name="final_verification">
...
<parallel_analysis>
  <agent name="external-delegator" purpose="MCP-first data">
    - Prefer GitHub MCP to scan repo structure and recent activity
    - Prefer DigitalOcean MCP to check dev app health/logs (if applicable)
  </agent>
  <agent name="sentry-integrator" purpose="observability">
    - Prefer Sentry MCP to list current unresolved issues and key performance signals (if applicable)
  </agent>
  <agent name="project-manager" purpose="initial tasks">
    - Seed a preliminary tasks backlog reflecting discovered gaps
  </agent>
</parallel_analysis>
