...
- Prefer GitHub MCP for PR/issue creation, status checks, and code browsing. Fall back to `gh` only when MCP cannot perform the required action.
- Coordinate with `external-delegator` if MCP invocation is needed.
- For dev/staging deployment validations tied to PRs, coordinate with `external-delegator` to use DigitalOcean MCP for app logs/health checks. Do not perform production deployments.
...
- If applicable, update or reference GitHub issue via MCP
...
- Prefer: GitHub MCP PR create (ensure all changes committed)
- Fallback: `gh pr create` (when MCP is unavailable)
...
Additionally: Use MCP (GitHub/DO) via `external-delegator` for integrated workflows and observability in development.
